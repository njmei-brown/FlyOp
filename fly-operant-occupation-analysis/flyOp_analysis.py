...
def plot_train_test_occupancy_entries(train_df, test_df, title=None):
    ...
    p_val = wilcox_test(robjects.IntVector(train_df.loc['Occupancy ROI Entries'].values), robjects.IntVector(test_df.loc['Occupancy ROI Entries'].values)).rx2('p.value')[0]
    p_val_statement = "Train vs. Test Occupancy ROI Entries P-Value: {}".format(p_val)
    ...
    add_staggered_points(ax, 1, train_df.loc['Occupancy ROI Entries'].values, color='b', marker='o', alpha=0.3, linestyle='None')             
    add_staggered_points(ax, 2, test_df.loc['Occupancy ROI Entries'].values, color='b', marker='o', alpha=0.3, linestyle='None')
...
def plot_train_test_dwell_times(train_df, test_df, title=None):
    ...
    color_cycle = ['#e41a1c', '#377eb8', '#4daf4a', '#984ea3'] * (train_df.shape[1]/4)
    ...
    wilcox_test = robjects.r['wilcox.test']
    p_val = wilcox_test(robjects.FloatVector(train_df_hstack), robjects.FloatVector(test_df_hstack)).rx2('p.value')[0]
    p_val_statement = "Train vs. Test Occupancy ROI Dwell Times P-Value: {}".format(p_val)
    print(p_val_statement)  
...
def plot_train_test_total_dwell_time(train_df, test_df, title=None):
    ...
    wilcox_test = robjects.r['wilcox.test']
    p_val = wilcox_test(robjects.IntVector(train_df.loc['Trial Occupancy ROI Dwell Time'].values), robjects.IntVector(test_df.loc['Trial Occupancy ROI Dwell Time'].values)).rx2('p.value')[0]
    p_val_statement = "Train vs. Test Occupancy ROI Total Dwell Time P-Value: {}".format(p_val)
    print(p_val_statement)    
    ...
    add_staggered_points(ax, 1, train_df.loc['Trial Occupancy ROI Dwell Time'].values, color='b', marker='o', alpha=0.3, linestyle='None')             
    add_staggered_points(ax, 2, test_df.loc['Trial Occupancy ROI Dwell Time'].values, color='b', marker='o', alpha=0.3, linestyle='None')
...
def train_test_occupancy_comparisons(path_to_train_dir=None, path_to_test_dir=None):
    ...
    if path_to_train_dir and path_to_test_dir:
        ...
        plot_positions(path_to_train_dir, title=title + ' - Train')
        plot_positions(path_to_test_dir, title=title + ' - Test')
